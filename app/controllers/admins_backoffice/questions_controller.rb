class AdminsBackoffice::QuestionsController < AdminsBackofficeController
  ...
  def index
    respond_to do |format|
      format.html { @questions = Question.all.order(:description).page(params[:page]) }
      format.pdf  { @questions = Question.all.order(:description) }
      format.json { @questions = Question.all.order(:description) }
    end
  end
